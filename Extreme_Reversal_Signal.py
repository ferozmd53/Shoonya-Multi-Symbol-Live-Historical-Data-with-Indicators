# Extreme_Reversal_Signal.py - StochRSI + Bollinger Bands (Fixed Excel update)
...
def fetch_historical_data(symbol):
    try:
        ...
        ret = api.get_daily_price_series(exchange="NSE", tradingsymbol=symbol,
        ...
        df.rename(columns={'time': 'datetime', 'into': 'open', 'inth': 'high',
                           'intl': 'low', 'intc': 'close', 'intv': 'volume'}, inplace=True)
        ...
        return {'all_closes': df['close'].tolist(), 'yesterday': yesterday_data}
    except Exception as e:
        print(f"Error: {e}")
        return None
...
def setup_excel_headers():
    try:
        ...
        headers = ['Symbol', 'LTP', 'Open', 'High', 'Low', 'Close', 'Volume',
...
def update_excel_bulk():
    ...
    try:
        ...
        # Read symbols from column A (don't write to it)
        symbols_range = ws.range("A2:A200")
        symbols_list = symbols_range.value
        
        if not symbols_list:
            return
        
        # Prepare data for columns B to Q only
        data_rows = []
        
        for symbol_val in symbols_list:
            if not symbol_val:
                data_rows.append([''] * 15)  # B to P = 15 columns
                continue
        ...
        # Update ONLY columns B to P (NOT column A)
        if data_rows:
            ws.range(f"B2:P{2 + len(data_rows) - 1}").value = data_rows
...
def main():
    ...
    print("\nStarting WebSocket...")
    try:
        api.start_websocket(subscribe_callback=on_ticks, order_update_callback=on_order,
                           socket_open_callback=on_open, socket_close_callback=on_close)
        for _ in range(15):
            if feed_opened:
                break
            time.sleep(1)
